...
export function hotpGenerate(
    ...
    return ((hResult.readUInt32BE(hResult[MAGIC_BYTE_OFFSET[digest]] & 0x0F) & 0x7FFFFFFF) % codeWidth[digits]).
        toString()
        .padStart(digits, '0');
}
...
/**
 * @throws {TypeError} If the digits is not an integer between 4 and 10.
 * @internal
 */
export function checkDigits(digits: number): void {
    ...
    if (typeof digits !== 'number' || !Number.isSafeInteger(digits) || digits < 4 || digits > 10) {
        ...
        throw new TypeError('The digits must be an integer between 4 and 10.');
    }
}
 ...
 * @internal
 */
export function checkDigest(digest: unknown): asserts digest is cL.EDigest {
